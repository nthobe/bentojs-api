...
// ### Models
// Interface models are lazy loaded when the first token request hits the API.
...
Bento.Auth = function *(next) {
  let token = this.request.headers.authorization;
  let user  = null;
  if (token) {
    loadModels();
    user = yield Bento.Auth.getUser(token);
  }
  this.auth = new Auth(user, token);
  yield next;
};
...
Bento.Auth.getUser = function *(token) {
  ...
  let group     = yield Group.findById(connector.groupId);
  ...
  let role = yield Role.findById(groupRole.roleId);
  if (!role) {
  ...
  user.group = group;
  user.groupRole = groupRole; // new functionality needs this in context
  user.role  = {
...
/**
 * Lazy loads the required interface models.
 * @return {Void}
 */
function loadModels() {
